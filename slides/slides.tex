...
%\ifANIMATE
			\only<\the\numexpr\value{numSlide}>{
				$\Rightarrow$ une nouvelle \textit{étiquette} (\texttt{legumes}) apparait, elle pointe vers le même \texttt{commit} que \texttt{HEAD}
				\vspace{-1.65em}
			}
%\fi
\ifANIMATE
...
\ifANIMATE
	...
	\only<2>{\begin{tikzpicture}[remember picture,overlay]\node[xshift=1.3cm,yshift=2.1cm] at (current page.south west) {$\hookrightarrow$};\end{tikzpicture}}
\fi
\pause
\PAUSE
\begin{minted}[mathescape=true,escapeinside=||,tabsize=4,fontsize=\footnotesize,]{c}
...
\begin{block}{}
\begin{itemize}
\item À faire une seule fois: informations stockées dans \texttt{\textasciitilde/.gitconfig}
