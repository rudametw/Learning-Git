...
Pour commencer ce TP, nous vous fournissons une configuration avancé de \texttt{bash} (l'interpréteur en ligne de commande utilisé par défaut).
...
Cette disposition vous permettra d'exécuter vos commandes \texttt{git} dans le terminal \texttt{T1}, tout en exécutant des commandes de \textit{suivi} dans les terminaux \texttt{T2} et \texttt{T3}.~\ref{terminals}\\

Dans un premier temps, positionnez \texttt{T1}, \texttt{T2} et \texttt{T3} dans le répertoire où vous allez travailler.\\

La commande \texttt{watch}\footnote{\url{https://en.wikipedia.org/wiki/Watch\_(Unix)}} permet de répéter une commande toute les $X$ seconds. Dans le terminal \texttt{T2} exécutez :
\begin{minted}[mathescape=true,escapeinside=||,tabsize=4
%	,fontsize=\footnotesize,
]{bash}
...
Pour rappel, l'enchainement des commandes est : \mintinline{bash}{status, add, status, commit, status, log}.
...
\begin{itemize}
\item allez sur la branche \texttt{legumes}
\item vérifier l'état de votre espace de travail (e.g., l'inexistence du fichier \texttt{sauces.txt})
\item mergez \texttt{master} dans \texttt{legumes}
\item si tout c'est bien passé, mergez \texttt{legumes} dans \texttt{master}
...
\item Ajouter votre binôme aux membres de votre projet avec le niveau de droit \textit{Developer} (il devra se connecter une première fois avant d'apparaitre dans la liste de membres)
\end{itemize}
...
\begin{enumerate}
\item Aller dans les \textit{Settings} de votre projet (menu à gauche),
\item Cliquer sur \textit{Members} dans le menu \textit{Settings} (menu à gauche),
\item Cliquer sur \textit{New project member} en haut à droite,
...
Vous allez travaillez en même temps sur votre projet, chacun sur son propre dépôt local, et vous allez synchroniser les changements après.

\subsection{Merge sans conflit}

\paragraph{Travail de Binôme1}
\begin{itemize}
\item créer une branche \texttt{épices}
...
Un de vous deux a été le premier à pousser, et l'autre a du merger les changements avant de pouvoir pousser son travail.
...
\begin{itemize}
\item éditez tous les deux, chacun sur son dépôt, le fichier \texttt{fruits.txt}, de façon à effacer certains fruits et à ajouter des nouveaux (faites des changements différents)
...
\item une fois résolut, merger vos changements et poussez vers le serveur
...
\begin{alertinfo3}{Astuce !}
Pour minimiser les conflits, il faut s'assurer que tout le monde maintient son dépôt à jour.
...
Vous pouvez vous inspirer de cette template sur Github :\\
...
Si vous voulez maitriser \texttt{git}, n'hésitez pas à faire le tutoriel donné par le laboratoire CRIStAL de Lille.
Il y a beaucoup de fonctionnalités avancés qui peuvent s'avérer très utiles.
