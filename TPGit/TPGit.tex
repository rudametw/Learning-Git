...
%\pagestyle{empty}
\title{Spécialités SE et IS 3$^{\mbox{\`eme}}$ année\\
...
Si vous ne connaissez pas une commande, \underline{consultez la documentation}.
\end{alertinfo}
...
Pour ne pas devoir lancer cette commande à chaque connexion, vous pouvez l'insérer au début de votre fichier \texttt{\~{}/.bashrc} qui sert à configurer l'interpreteur de commandes \textit{bash}.
...
Dans un premier temps, positionnez \texttt{T1}, \texttt{T2} et \texttt{T3} dans le répertoire où vous allez travailler via la commande \texttt{cd} et adaptez la taille de police.\\

La commande \texttt{watch}\footnote{\url{https://en.wikipedia.org/wiki/Watch\_(Unix)}} permet de répéter une commande toutes les $X$ secondes et la commande \texttt{tree} permet de visualiser toute l'arborescence de dossiers et fichiers. Dans le terminal \texttt{T2} nous allons surveiller le répertoire \texttt{tpgit} (qui n'existe pas encore), alors exécutez :
\begin{minted}[mathescape=true,escapeinside=||,tabsize=4
%	,fontsize=\footnotesize,
]{bash}
...
\end{minted}
%Already defaults :
%	git config |-{}-|global push.default simple #default already
%Unsure about this one ?
%	git config |-{}-|global log.decorate full #auto is default
%	git config |-{}-|global color.ui always #auto is default
% This one is useful :
...
Il faut régulièrement faire \texttt{git pull} sur vos branches et les merger, et corriger les éventuels petits conflits quand ils sont simples.
...
\section{Profitez de la puissance des alias}

N'oubliez pas, Git supporte des alias pour faciliter l'utilisation de commandes complexes.
Créer un alias pour \texttt{git log}, par exemple, avec la commande suivante :
\begin{minted}[mathescape=true,escapeinside=||,tabsize=4
%	,fontsize=\footnotesize,
]{bash}
	git config |-{}-|global alias.l 'log ----graph ----oneline ----decorate ----all'
\end{minted}
vous pouvez maintenant exécuter \textbf{\texttt{git l}} dans votre dépôt pour avoir un log joli.
